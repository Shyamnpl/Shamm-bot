...
async def xSEndMsg(Msg , Tp , Tp2 , id , K , V):
    feilds = {1: id , 2: Tp2 , 3: Tp, 4: Msg, 5: int(time.time()), 7: 2, 9: {1: "MG24-BOT", 2: int(await xBunnEr()), 3: 901048018, 4: 330, 5: 909034009, 8: "MG24-BOT", 10: 1, 11: 1, 13: {1: 2}, 14: {1: 12484827014, 2: 8, 3: "\u0010\u0015\b\n\u000b\u0013\f\u000f\u0011\u0004\u0007\u0002\u0003\r\u000e\u0012\u0001\u0005\u0006"}, 12: 0}, 10: "en", 13: {3: 1}}
...
async def xSEndMsgsQ(Msg , id , K , V):
    fields = {1: id , 2: id , 4: Msg , 5: int(time.time()), 7: 2, 8: 904990072, 9: {1: "MG24-BOT", 2: await xBunnEr(), 4: 330, 5: 827001005, 8: "MG24-BOT", 10: 1, 11: 1, 13: {1: 2}, 14: {1: 1158053040, 2: 8, 3: "\u0010\u0015\b\n\u000b\u0015\f\u000f\u0011\u0004\u0007\u0002\u0003\r\u000e\u0012\u0001\u0005\u0006"}}, 10: "en", 13: {2: 2, 3: 1}}
...
# --- [NEW] START CLASH SQUAD GAME FUNCTION --- #
async def STarT_GaME_CS(K, V, region):
    """
    Creates a packet to start a Clash Squad (Normal) game.
    NOTE: The field values (like 2:{1:1, 2:10}) are based on common patterns.
    If this doesn't work, these numbers may need to be changed after analyzing game traffic.
    """
    fields = {
        1: 9,
        2: {
            1: 1,  # Often represents the Map ID
            2: 10  # Often represents the Game Mode ID (10 is a common guess for CS)
        }
    }
    if region.lower() == "ind": packet_header = '0514'
    elif region.lower() == "bd": packet_header = "0519"
    else: packet_header = "0515"
    return await GeneRaTePk((await CrEaTe_ProTo(fields)).hex(), packet_header, K, V)
# -------------------------------------------- #

async def JoIn_GuiLD(GuiLD_iD, K, V):
    fields = {1: 14, 2: {1: int(GuiLD_iD)}}
